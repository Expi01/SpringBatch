...
public class SpringBatchConfig {
	...
	public Job job(JobRepository jobRepository, PlatformTransactionManager transactionManager,
		...
		Step step = new StepBuilder("Task-File-1", jobRepository).<Employee, Employee>chunk(100, transactionManager)
				.reader(itemReader).processor(itemProcessor).writer(itemWriter).build();

		logger.info("Step 'Task-File-1' configured.");

		return new JobBuilder("Task-1", jobRepository).incrementer(new RunIdIncrementer()).start(step).build();
	}
